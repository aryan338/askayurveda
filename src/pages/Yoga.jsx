            ...
            <div className="container mx-auto px-4 md:px-6">
                <div className="max-w-4xl mx-auto">
                    <div className="text-center mb-16 animate-in fade-in slide-in-from-bottom-4 duration-700">
                        <h1 className="text-5xl font-bold text-gray-900 mb-4">Yoga for Healthy Living</h1>
                        <p className="text-xl text-gray-600 italic">"Connecting the internal and external energies for a balanced life."</p>
                    ...
                    <div className="grid md:grid-cols-3 gap-8 mb-20 animate-in fade-in duration-1000 delay-200">
                        {principles.map((p, i) => (
                            <div key={i} className="bg-white p-8 rounded-3xl shadow-sm border border-emerald-50 hover:shadow-xl transition-shadow text-center">
                                <div className="w-12 h-12 bg-emerald-100 text-emerald-600 rounded-2xl flex items-center justify-center mx-auto mb-6">
                                    {p.icon}
                                </div>
                                <h3 className="text-xl font-bold text-gray-900 mb-3">{p.title}</h3>
                                <p className="text-gray-600 leading-relaxed text-sm">{p.text}</p>
                            </div>
                        ))}
                    </div>

                    <div className="bg-white rounded-[2.5rem] p-8 md:p-16 shadow-xl border border-stone-100 relative overflow-hidden mb-16 animate-in zoom-in duration-700 delay-300">
                        ...
                        <div className="grid sm:grid-cols-2 gap-x-12 gap-y-8">
                            {limbs.map((limb, i) => (
                                <div key={i} className="flex gap-4 group">
                                    <div className="bg-emerald-50 text-emerald-700 w-10 h-10 rounded-xl flex items-center justify-center shrink-0 font-bold group-hover:bg-emerald-600 group-hover:text-white transition-colors">
                                        {i + 1}
                                    </div>
                                    <div>
                                        <h4 className="font-bold text-gray-900 mb-1">{limb.title}</h4>
                                        <p className="text-gray-500 text-sm leading-relaxed">{limb.desc}</p>
                                    </div>
                                </div>
                            ))}
                        </div>
                    ...
                    <div className="bg-emerald-900 rounded-[2.5rem] p-10 md:p-16 text-white text-center relative overflow-hidden">
